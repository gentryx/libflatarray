...
#undef CASE
        ...
        void bind_size_z(const std::size_t dim_z, char *data, FUNCTOR& functor)
        {
#define CASE(SIZE)                                                      \
            if (dim_z <= SIZE) {                                        \
...
#undef CASE
    ...
    template<typename T, typename HAS_SIZES = void>
    class select_sizes : public has_default_3d_sizes
    {};

    template<typename T>
    class select_sizes<T, typename T::API::has_sizes>
    {
    public:
    };
